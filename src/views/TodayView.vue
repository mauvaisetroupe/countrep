...
// Date actuelle réelle pour identifier le "vrai" jour d'aujourd'hui dans le calendrier
...
const currentMonth = computed(() => {
  const dateObj = new Date(currentYear.value, currentMonthIndex.value, 1)
...
const daysInMonth = computed(() => {
  ...
  let firstDayIndex = new Date(currentYear.value, currentMonthIndex.value, 1).getDay()
  ...
  const totalDays = new Date(currentYear.value, currentMonthIndex.value + 1, 0).getDate()
...
const saveWorkout = async () => {
  ...
  const formattedDate = `${currentYear.value}-${String(currentMonthIndex.value + 1).padStart(2, '0')}-${String(selectedDate.value).padStart(2, '0')}`
  ...
  <main class="px-5 mt-4 space-y-4">
    ...
    <section class="bg-amber-50/40 border border-amber-100/80 rounded-3xl p-5 shadow-xs">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-lg font-bold capitalize text-gray-800">{{ currentMonth }}</h2>
        <div class="flex gap-1">
          <button @click="prevMonth" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-amber-200/50 text-gray-700 transition-colors">
            ‹
          </button>
          <button @click="nextMonth" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-amber-200/50 text-gray-700 transition-colors">
            ›
          </button>
        </div>
      </div>
        ...
        <div v-for="(day, index) in daysInMonth" :key="index" class="flex justify-center items-center">
          <button 
            ...
            :class="[
              ...
              (day === realDay && currentMonthIndex === realMonthIndex && currentYear === realYear) ? 'border border-amber-600 text-gray-900' : 'text-gray-700 hover:bg-amber-100/50'
            ]"
          >
            {{ day }}
          </button>
